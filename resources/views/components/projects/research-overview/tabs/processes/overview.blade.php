...
@php
    $totalProcesses = (int) ($metrics['totalProcesses'] ?? 0);
    $readinessPercent = (int) ($metrics['readinessPercent'] ?? 0);
    $needsReviewCount = collect($metrics['processesNeedingReview'] ?? [])->count();
    ...
    <div class="card-body p-3 background-white">
        <div class="d-flex flex-wrap align-items-start justify-content-between gap-2 mb-3">
            <div>
                ...
                <p class="text-muted mb-0" style="font-size:.85rem;">
                    Process readiness based on metadata, files, samples, studies, datasets, and measurements.
                </p>
            </div>
        ...
        <div class="row g-2 mb-3">
            <div class="col-6 col-lg-4">
                <div class="border rounded p-2 h-100">
                    ...
                    <div class="fw-bold text-secondary">{{ number_format($totalProcesses) }}</div>
                </div>
            </div>
            ...
            <div class="col-6 col-lg-4">
                <div class="border rounded p-2 h-100">
                    ...
                    <div class="fw-bold text-secondary">{{ number_format((int) ($metrics['totalAttributes'] ?? 0)) }}</div>
                </div>
            </div>
            ...
            <div class="col-6 col-lg-4">
                <div class="border rounded p-2 h-100">
                    ...
                    <div class="fw-bold text-success">{{ number_format((int) ($metrics['totalSamples'] ?? 0)) }}</div>
                </div>
            </div>
        ...
        <div class="border rounded p-3 bg-light">
            <div class="fw-semibold small mb-1">
                <i class="fas fa-lightbulb text-warning me-1"></i>Recommended Process Action
            </div>

            @if($totalProcesses === 0)
                <p class="text-muted mb-2" style="font-size:.85rem;">
                    This project does not have processes yet. Processes usually come from studies or imported experiment data.
                </p>
            ...
            @elseif($needsReviewCount > 0)
                <p class="text-muted mb-2" style="font-size:.85rem;">
                    Some processes are missing files, samples, studies, attributes, descriptions, or type information.
                </p>
                <a href="{{ route('projects.data-dictionary.activities', [$project]) }}"
                   class="btn btn-sm btn-outline-secondary">
                    <i class="fas fa-list me-1"></i>Review Process Attributes
                </a>
            @else
                <p class="text-muted mb-2" style="font-size:.85rem;">
                    Process readiness looks good from available counts. Review process attributes as the project evolves.
                </p>
                <a href="{{ route('projects.data-dictionary.activities', [$project]) }}"
                   class="btn btn-sm btn-outline-secondary">
                    <i class="fas fa-list me-1"></i>View Process Attributes
                </a>
            @endif
        </div>
    </div>
