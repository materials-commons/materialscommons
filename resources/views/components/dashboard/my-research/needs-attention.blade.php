@php
    $needsAttentionKey = 'mc_dashboard_my_research_needs_attention';
@endphp

<div class="d-flex align-items-center mb-2">
    <button class="btn btn-link btn-sm p-0 text-decoration-none text-muted d-flex align-items-center gap-2"
            type="button"
            id="my-research-needs-attention-toggle"
            data-bs-toggle="collapse"
            data-bs-target="#my-research-needs-attention"
            aria-expanded="false"
            aria-controls="my-research-needs-attention">
        <i class="fas fa-chevron-right fa-fw"
           id="my-research-needs-attention-chevron"
...
<div class="collapse mb-3" id="my-research-needs-attention">
...
@push('scripts')
    <script>
        (function () {
            const SECTION_STORAGE_KEY = @json($needsAttentionKey);
            const sectionPanel = document.getElementById('my-research-needs-attention');
            const sectionToggle = document.getElementById('my-research-needs-attention-toggle');
            const sectionChevron = document.getElementById('my-research-needs-attention-chevron');

            if (sectionPanel) {
                if (localStorage.getItem(SECTION_STORAGE_KEY) === 'true') {
                    ...
                    if (sectionChevron) {
                        sectionChevron.style.transform = 'rotate(90deg)';
                    }
                }

                sectionPanel.addEventListener('show.bs.collapse', () => {
                    localStorage.setItem(SECTION_STORAGE_KEY, 'true');
                ...
                sectionPanel.addEventListener('hide.bs.collapse', () => {
                    localStorage.setItem(SECTION_STORAGE_KEY, 'false');

                    if (sectionChevron) {
                        sectionChevron.style.transform = 'rotate(0deg)';
                    }
                });
            }
