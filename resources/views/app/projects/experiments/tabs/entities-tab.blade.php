...
@php
    ...
    $projAnalyticsKey = 'mc_proj_' . $project->id . '_' . ($category ?? 'exp') . '_samples_analytics_tab';
...
<x-collapsible-section title="KPI" id="samples-kpi" storage-key="proj_exp_samples_kpi">
...
@if($totalProcesses > 0 || count($samplesByStudy) > 0)
    <x-collapsible-section id="samples-analytics"
                           title="Analytics"
                           :resize-plotly="true"
                           storage-key="proj_exp_samples_analytics">
        <div class="row g-3">
            @if($totalProcesses > 0)
