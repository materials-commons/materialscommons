...
<a href="#" onclick="mcutil.copyToClipboard('{{$file->fullPath()}}')" class="ms-2"><i class="fa fas fa-clone"></i></a>
@if($fileExists($file))
    @switch($fileType($file))
        @case("text")
            @if(isset($displayRoute))
                <a class="ms-4" href="{{$displayRoute}}">Fullscreen</a>
            @endif
            @if($file->size <= 2000000)
                <a href="#" onclick="mcutil.copyToClipboard('#file-contents')" class="ms-4">
                    copy file contents
                </a>
            @endif
            @if(isset($editRoute))
                <a href="{{$editRoute}}" class="ms-4">Edit</a>
            @endif
            @break
